...
async fn list_rooms(State(state): State<AppState>) -> Result<impl IntoResponse, AppError> {
    let db = state.db();
    let rooms = store::list_rooms(&db)?;
...
async fn mark_seen(
    ...
    let db = state.db();
    Ok(match store::mark_room_seen(&db, &id)? {
        true => Json(json!({ "ok": true })).into_response(),
        false => (
            StatusCode::NOT_FOUND,
            Json(json!({"error": "no such group chat"})),
...
async fn mark_unseen(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Response, AppError> {
    let db = state.db();
    Ok(match store::mark_room_unread(&db, &id)? {
        true => Json(json!({ "ok": true })).into_response(),
        false => (
            StatusCode::NOT_FOUND,
            Json(json!({"error": "no such group chat"})),
